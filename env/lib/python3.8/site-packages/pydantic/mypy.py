...
class PydanticModelTransformer:
    ...
    def add_initializer(
        ...
        with state.strict_optional_set(self._api.options.strict_optional):
            ...
            if is_settings:
                base_settings_info = self._api.lookup_fully_qualified(
                    BASESETTINGS_FULLNAME
                ).node.defn.info
                base_settings_init_arguments = base_settings_info.names[
                    "__init__"
                ].node.arguments
                settings_init_arguments = []
                a: Argument
                for a in base_settings_init_arguments:
                    if a.variable.name.startswith(
                        "__"
                    ) or not a.variable.name.startswith("_"):
                        continue
                    analyzed_variable_type = self._api.anal_type(a.variable.type)
                    variable = Var(a.variable.name, analyzed_variable_type)
                    settings_init_arguments.append(
