...
class DecimalValidator:
    ...
    def __post_init__(self) -> None:
        self.check_digits = (
            self.max_digits is not None or self.decimal_places is not None
        )
        ...
        self.multiple_of = (
            decimal.Decimal(self.multiple_of) if self.multiple_of is not None else None
        )
    ...
    def __get_pydantic_core_schema__(
        ...
        json_schema = core_schema.no_info_after_validator_function(
            to_decimal, primitive_schema
        )
        schema = core_schema.json_or_python_schema(
            json_schema=json_schema,
            python_schema=core_schema.lax_or_strict_schema(
                lax_schema=core_schema.union_schema(
                    [core_schema.is_instance_schema(decimal.Decimal), json_schema]
                ),
        ...
        if self.multiple_of is not None:
            schema = core_schema.no_info_after_validator_function(
                partial(
                    _validators.multiple_of_validator, multiple_of=self.multiple_of
                ),
                schema,
            )
