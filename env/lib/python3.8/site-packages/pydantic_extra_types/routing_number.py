...
class ABARoutingNumber(str):
    ...
    def validate_digits(cls, routing_number: str) -> None:
        if not routing_number.isdigit():
            raise PydanticCustomError(
                "aba_routing_number", "routing number is not all digits"
            )
    ...
    def validate_routing_number(cls, routing_number: str) -> str:
        ...
        checksum = (
            3
            * (sum(map(int, [routing_number[0], routing_number[3], routing_number[6]])))
            + 7
            * (sum(map(int, [routing_number[1], routing_number[4], routing_number[7]])))
            + sum(map(int, [routing_number[2], routing_number[5], routing_number[8]]))
        )
        if checksum % 10 != 0:
            raise PydanticCustomError(
                "aba_routing_number", "Incorrect ABA routing transit number"
            )
